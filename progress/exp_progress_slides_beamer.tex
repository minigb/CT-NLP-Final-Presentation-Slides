...
\begin{frame}{EXP034B: Music-Claim Hardening -- Complete}
  \footnotesize
  \expfield{EXP Objective}{Open-gate matched comparison: reason-only versus aligned, shuffled, and dummy \code{S\_plan} streams under identical LoRA / data / step budget (800 steps, seed 260635). All four conditions complete.}

  \begin{tabularx}{\linewidth}{p{3.1cm}p{1.35cm}p{1.35cm}p{1.4cm}Y}
    \toprule
    Condition & Test CE & Test PPL & Test Acc & $\Delta$ CE vs aligned \\
    \midrule
    Reason-only baseline & 5.8590 & 350.4 & 0.1039 & $+$0.259 \\
    Aligned \code{S\_plan} stream & \textbf{5.5998} & \textbf{270.4} & \textbf{0.1207} & \textemdash \\
    $\hookrightarrow$ Shuffled \code{S\_plan} & 5.6144 & 274.3 & 0.1191 & $+$0.015 \\
    $\hookrightarrow$ Dummy stream & 5.6143 & 274.3 & 0.1193 & $+$0.015 \\
    \bottomrule
  \end{tabularx}

  \vspace{0.18cm}
  \begin{keybox}\textbf{Claim (paper-safe):} Aligned \code{S\_plan} beats both alignment-corrupted controls by $\approx 0.015$ CE and the no-symbolic reason-only baseline by $0.259$ CE. Gate value is higher for aligned (0.177) than shuffled (0.167) or dummy (0.163), consistent with active symbolic use. \code{claim\_ready = true}.\end{keybox}

  \vspace{0.08cm}
  \expfield{Claim boundary}{EXP034B is semantic-token stream training under oracle symbolic input. It is not text-to-music, song-generation, or MIDI-to-audio generation.}
\end{frame}
...
\begin{frame}{Current Paper Story}
  ...
  \begin{enumerate}
    ...
    \item Aligned \code{S\_plan} improves acoustic-token prediction compared with shuffled/dummy controls (EXP025).
    \item Naive LALM insertion fails; a stream-native warmup (EXP029) gives a small positive control result.
    \item MIR-supervised EXP031A suggests \code{S\_plan} contains structured musical attributes, with axis-level caveats.
    \item \textbf{EXP034B (complete):} In the hardened open-gate comparison with all four matched conditions, aligned \code{S\_plan} achieves test CE $5.600$ vs.\ shuffled $5.614$, dummy $5.614$, and reason-only $5.859$. This is the strongest controlled positive result in the ladder.
  \end{enumerate}
\end{frame}
...
\begin{frame}{Most Important Next Experiments}
  ...
  \begin{tabularx}{\linewidth}{p{1.2cm}p{3.3cm}Yp{3.3cm}}
    ...
    \midrule
    P0 & EXP034B controls & Makes aligned vs shuffled vs dummy stream comparison paper-safe & \textbf{Complete:} aligned 5.600 $<$ shuffled 5.614 $<$ dummy 5.614 $<$ reason-only 5.859 \\
...
\begin{frame}{Team Handoff Wording}
  ...
  \begin{block}{What we can say}
    We have a compact learned symbolic planning codec, \code{S\_plan}, that (1) reconstructs MIDI-derived structure faithfully (EXP024), (2) improves acoustic-token prediction under aligned-vs-corrupted controls in three independent experiments (EXP025, EXP029, EXP034B), and (3) supports MIR structure recovery on pitch-class, density, and key axes pending label-shuffle controls (EXP031A). EXP034B is the strongest result: all four matched conditions complete, aligned beats both controls by $\approx 0.015$ CE.
  \end{block}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\linewidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{EXP034B complete:} aligned 5.600 $<$ both controls 5.614 $<$ reason-only 5.859; gate opens for aligned.
        \item EXP031A MIR recovery baseline (pitch-class, density, key).
      \end{itemize}
    \end{column}
    \begin{column}{0.48\linewidth}
